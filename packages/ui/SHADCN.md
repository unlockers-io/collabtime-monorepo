...
Put product-specific compositions and adapters in `src/compositions`, helpers in `src/lib`, and hooks in `src/hooks`. For example, `FormFieldError` converts arbitrary TanStack validator values to the upstream `FieldError` message shape. `StatusBadge` composes the upstream badge variant factory with product status colors. Use normal headings inside `CardTitle`; its upstream element is a generic div. Import `toast` from `sonner`.
