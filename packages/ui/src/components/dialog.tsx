...
function DialogOverlay({ className, ...props }: DialogPrimitive.Backdrop.Props) {
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        "ui:fixed ui:inset-0 ui:isolate ui:z-50 ui:bg-black/10 ui:duration-100 ui:data-closed:animate-out ui:data-closed:fade-out-0 ui:data-open:animate-in ui:data-open:fade-in-0 ui:supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "ui:fixed ui:top-1/2 ui:left-1/2 ui:z-50 ui:grid ui:w-full ui:max-w-[calc(100%-2rem)] ui:-translate-x-1/2 ui:-translate-y-1/2 ui:gap-4 ui:rounded-xl ui:bg-popover ui:p-4 ui:text-sm ui:text-popover-foreground ui:ring-1 ui:ring-foreground/10 ui:duration-100 ui:outline-none ui:data-closed:animate-out ui:data-closed:fade-out-0 ui:data-closed:zoom-out-95 ui:data-open:animate-in ui:data-open:fade-in-0 ui:data-open:zoom-in-95 ui:sm:max-w-sm",
          className,
        )}
        {...props}
      >
...
function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn("ui:text-base ui:leading-none ui:font-medium", className)}
      {...props}
    />
  );
}
