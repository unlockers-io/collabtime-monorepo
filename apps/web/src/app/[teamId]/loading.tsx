  ...
  return (
    <div className="min-h-dvh w-full px-4 py-6 sm:px-6 lg:px-8 xl:px-12">
      <main className="mx-auto flex w-full max-w-450 flex-col gap-6" id="main">
        <header className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="size-9 bg-muted motion-safe:animate-pulse" />
              <div className="h-7 w-40 bg-muted motion-safe:animate-pulse sm:h-8" />
            </div>
            <div className="hidden items-center gap-2 sm:flex">
              <div className="h-9 w-36 bg-muted motion-safe:animate-pulse" />
              <div className="h-8 w-24 bg-muted motion-safe:animate-pulse" />
              <div className="size-8 bg-muted motion-safe:animate-pulse" />
              <div className="size-8 bg-muted motion-safe:animate-pulse" />
            </div>
        ...
        <section className="flex flex-col gap-4 border-y border-border bg-transparent py-4 text-foreground sm:py-5">
          ...
          <div className="grid gap-3 px-4 sm:grid-cols-2 sm:px-5 lg:grid-cols-3">
            {["insight-1", "insight-2", "insight-3"].map((id) => (
              <div
                className="flex flex-col gap-2.5 border-t border-border bg-transparent p-3.5"
                key={id}
              >
              ...
              {["row-1", "row-2", "row-3"].map((id) => (
                <div className="flex items-center gap-2 sm:gap-3" key={id}>
                  <div className="flex w-28 shrink-0 items-center gap-2 sm:w-40">
                    <div className="size-6 bg-muted motion-safe:animate-pulse sm:size-7" />
                    <div className="hidden h-4 w-16 rounded bg-muted motion-safe:animate-pulse sm:block" />
                  </div>
                  <div className="h-8 flex-1 bg-muted motion-safe:animate-pulse" />
                </div>
              ))}
            </div>
          </div>
          ...
          <section className="flex flex-col gap-4 border-y border-border bg-transparent py-4 text-foreground sm:py-5">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(min(240px,100%),1fr))] gap-4 pr-8 pl-4 sm:pr-9 sm:pl-5">
              {["member-1", "member-2", "member-3", "member-4"].map((id) => (
                <div
                  className="flex h-full flex-col gap-3 border-t border-border bg-transparent p-4"
                  key={id}
                >
